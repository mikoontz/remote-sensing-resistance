...
sup2 <- readPNG("figs/heterogeneity-demo/suppressed_2-pixel.png")
sup3 <- readPNG("figs/heterogeneity-demo/suppressed_3-pixel.png")
sup4 <- readPNG("figs/heterogeneity-demo/suppressed_4-pixel.png")
man1 <- readPNG("figs/heterogeneity-demo/fire-use_1-pixel.png")
man2 <- readPNG("figs/heterogeneity-demo/fire-use_2-pixel.png")
man3 <- readPNG("figs/heterogeneity-demo/fire-use_3-pixel.png")
man4 <- readPNG("figs/heterogeneity-demo/fire-use_4-pixel.png")
...
pdf("figs/heterogeneity-imagery-histograms.pdf", width = 17.3 / 2.54, height = 0.75 * 17.3 / 2.54)
...
plot(density(s$het_ndvi_1), main = NA, xaxt = "n", las = 1, xlab = NA, ylab = NA)
axis(side = 1, at = d$het_ndvi_1, labels = round(d$het_ndvi_1, 2), las = 2, font = 2)
axis(side = 1, at = c(0, max(s$het_ndvi_1 / 2), max(s$het_ndvi_1)), labels = c(0, round(max(s$het_ndvi_1 / 2), 2), round(max(s$het_ndvi_1), 2)), las = 2)
abline(v = d[d$type == "fire_use", "het_ndvi_1"], col = my_colors[1], lwd = 2)
abline(v = d[d$type == "suppressed", "het_ndvi_1"], col = my_colors[2], lwd = 2)
legend("topright", legend = c("Groupy-gappy forest", "Dense, homogenous forest"), col = my_colors, lty = 1, lwd = 3, bty = "n")
mtext(side = 2, text = "Density", outer = TRUE, at = 1/6, cex = 1)
...
# legend("topright", legend = c("Groupy-gappy forest", "Dense, homogenous forest"), col = my_colors, lty = 1, lwd = 3)
...
# legend("topright", legend = c("Groupy-gappy forest", "Dense, homogenous forest"), col = my_colors, lty = 1, lwd = 3)
mtext(side = 1, text = "Heterogeneity of NDVI", outer = TRUE, line = 0, cex = 1)
dev.off()