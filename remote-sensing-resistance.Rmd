...
What is the effect of heterogeneity of forest structure on wildfire severity
...
### Collation and processing of pre- and post-fire imagery
...
We calculated remotely-sensed wildfire using the relative burn ratio (RBR) [@Parks2014], the delta normalized burn ratio (dNBR), the relative differenced normalized burn ratio (RdNBR), the delta normalized burn ratio 2 (dNBR2), and the relative differenced normalized burn ratio 2 (RdNBR2).

$$
NBR = \frac{NIR - SWIR}{NIR + SWIR }
$$
Where NIR is the near infrared band (band 4 on Landsat 4, 5, and 7; band 5 on Landsat 8) and SWIR is the short wave infrared band (band 7 on Landsat 4, 5, 7, and 8)

### Calibrating remotely-sensed wildfire severity with field-measured wildfire severity

We calibrated our remotely-sensed measure of wildfire severity with field measures of overstory tree mortality from two previously published studies [@Zhu2006; @Sikkink2013]. The Composite Burn Index (CBI) is a metric of change in vegetation across several vertical strata [@Key2006] and has a long history of use in calibrating remotely-sensed severity data [@Miller2007; @Miller2009a; @Cansler2012; @Parks2014; @Prichard2014]. 

$$
\text{remote\_severity} = \beta_0 + \beta_1 e^{\beta_2 \text{cbi\_overstory}}
$$

We applied the image collation, image processing, and wildfire severity calculation workflow to 402 previously-published Composite Burn Index plotUpon We used the Relative Burn Ratio 
...
$$
\begin{aligned}
severity_i \sim \mathcal{N}(\mu, \sigma)
\end{aligned}
$$
