...
We calculated the delta severity indices by subracting the postfire index from the prefire index without multiplying by a rescaling constant (e.g., we did not multiply the result by 1000 as in @Miller2007):
...
Following @Reilly2017, we chose not to correct the delta indices using a phenological offset value (typically calculated as the delta index in homogenous forest outside of the fire perimeter), instead choosing to account for phenology by using a per-pixel median reducer across multiple cloud-free images captured during the same time window just before the fire and one year later.  

We calculated the relative delta severity indices by scaling the delta index from Eq. \ref{eq-dindex} by a square root transformation of the absolute value of the prefire index:
