...
bibliography: "../ms/ms_carpentry/remote-sensing-resistance.bib"
csl: "../ms/ms_carpentry/ecology-letters.csl"
---
...
```{r read_files, cache = TRUE, message = FALSE}
...
```{r explore_fires_raw, cache = TRUE}
glimpse(fires)
fires %>% 
  filter(YEAR_ >= 1982) %>% 
  group_by(YEAR_) %>% 
...
```{r build_models_continuous, cache = TRUE}
...
```{r response_correlations, cache = TRUE}
...
model_summary <- read.csv("../data_output/cbi_calibration_model_comparison.csv", stringsAsFactors = FALSE)
...
```{r build_model_binomial, cache = TRUE}
...
extreme_glm_1 <- glmer(stand_replacing ~ het_ndvi_1_s +
                                 fm100_s +
                                 extreme80_fm100 +
                                 het_ndvi_1_s:extreme80_fm100 + 
                                 het_ndvi_1_s:fm100_s +
...
summary(extreme_lm_1)
summary(extreme_glm_1)
```
