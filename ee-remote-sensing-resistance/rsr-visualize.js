...
var RBR_viz = { min: 0.041191844, max: 0.2836425, 
...
var hamm_img = rsr.get_variables(hamm, timeWindow, resample_method, sats).clip(hamm);
...
// Map.addLayer(hamm_img.select(['preFire_ndvi']), {}, 'Prefire NDVI');
// Map.addLayer(hamm_img.select(['postFire_ndvi']), {}, 'Postfire NDVI');
// Map.addLayer(hamm_img.select(['preFire_nbr']).updateMask(hamm_img.select(['conifer_forest'])), {}, 'Prefire NBR');
// Map.addLayer(hamm_img.select(['postFire_nbr']).updateMask(hamm_img.select(['conifer_forest'])), {}, 'Postfire NBR');

// print(hamm_img.float().bandNames());
...
// var hamm_pt = ee.Geometry.Point([-120.0823, 37.7997]);
...
var american_img = rsr.get_variables(american, timeWindow, resample_method, sats).clip(american);
...
// var king_img = rsr.get_variables(king, timeWindow, resample_method, sats).clip(king);
...
var hwy_img = rsr.get_variables(hwy, timeWindow, resample_method, sats).clip(hwy);
...
var mcnally_img = rsr.get_variables(mcnally, timeWindow, resample_method, sats).clip(mcnally);
