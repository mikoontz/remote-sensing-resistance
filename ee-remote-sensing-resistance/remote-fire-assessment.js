...
// The image should already be clipped to the fire perimeter, so when the sample method uses
// the image footprint as its default region, that will work well
var get_stratified_samps = function(img) {
  ...
  var s = img.stratifiedSample({
          ...
          scale: 30,
          classValues: [0, 1],
          classPoints: [non_conifer_samps, conifer_samps],
          seed: 727,
          dropNulls: true
  });
...
// How many fires with imagery available (currently just using Landsat 5) both
// before and 1 year after the fire, such that we can get its severity/weather/fuel variables?
// Whoops, computation timed out. Skip it.
...
// var hamm_img = ee.Image(hamm.map(assess_whole_fire, true).first());
// var RBR_viz = {min: 0.036, max: 0.29, palette:['008000', 'ffff00',  'ffA500', 'ff0000']};
// Map.addLayer(hamm_img.select(['RBR']), RBR_viz);
// Map.centerObject(hamm_img);
// var hamm_samps = hamm_assessed.map(get_samps, true);
// var hamm_strat_samps = hamm_assessed.map(get_stratified_samps, true);
// print(hamm_strat_samps.flatten().getInfo());
// var hamm_samp_geo = get_stratified_samps(hamm_img);
// print(hamm_samp_geo.getInfo());
// var fire_samps_description = "hamm-strat-samples_" + timeWindow + "-month-window_L5_" + resample_method + "-interp";
// Export.table.toDrive({
//   'collection': hamm_strat_samps.flatten(),
//   'description': fire_samps_description,
//   'folder': 'ee',
//   'fileNamePrefix': fire_samps_description,
//   'fileFormat': 'GeoJSON'
...
// var end = ee.Date('2011-08-01').millis();
...
// var test_strat_samps = test_fire_assessments.map(get_stratified_samps, true);

// var fire_samps_description = "test-strat-samples_" + timeWindow + "-month-window_L5_" + resample_method + "-interp";

// Export.table.toDrive({
//   'collection': test_strat_samps.flatten(),
//   'description': fire_samps_description,
//   'folder': 'ee',
//   'fileNamePrefix': fire_samps_description,
//   'fileFormat': 'GeoJSON'
